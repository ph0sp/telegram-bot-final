...
async def error_handler(update: Update, context) -> None:
    ...
    logger.error(f"❌ Ошибка в боте: {error_str}", exc_info=True)


async def setup_application():
    """Настройка приложения и всех обработчиков"""
...
async def main():
    ...
    try:
        ...
        await application.run_polling(
            drop_pending_updates=True,  # Игнорировать сообщения, отправленные пока бот был офлайн
        ...
        logger.error(f"❌ Запуск бота не удался: {e}", exc_info=True)
        raise
    finally:
        ...
        # Если есть событие shutdown, устанавливаем его
        if shutdown_event:
            shutdown_event.set()
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.error(f"❌ Фатальная ошибка при запуске бота: {e}", exc_info=True)
