...
QUESTIONS = [
    ...
    # После ответа на первый вопрос бот должен сказать:
...
async def error_handler(update: Update, context: CallbackContext) -> None:
    """Обрабатывает ошибки бота без спама"""
    error = context.error
    
    # Игнорируем частые и неважные ошибки
    ignore_errors = [
    ...
    # Логируем все ошибки (но не спамим в Telegram)
    logger.error(f"❌ Ошибка в боте: {error}", exc_info=error)
    
    # Отправляем в Telegram только СЕРЬЕЗНЫЕ ошибки (1 раз в 10 минут)
    serious_errors = [
        "Token",
        "Authentication",
        "Forbidden",
    ...
    last_error_time = getattr(context, '_last_error_time', 0)
    
    # Проверяем, прошло ли 10 минут с последней отправки
    if current_time - last_error_time > 600:  # 600 секунд = 10 минут
        for serious in serious_errors:
            if serious in str(error):
                try:
                    error_msg = f"🚨 Серьезная ошибка:\n{type(error).__name__}: {error}"
                    ...
                    context._last_error_time = current_time
                    logger.info("✅ Уведомление об ошибке отправлено администратору")
                    break
