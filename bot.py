...
def analyze_user_profile(user_id: int) -> Dict[str, Any]:
    ...
    c.execute("SELECT question_number, answer_text FROM questionnaire_answers WHERE user_id = ?", (user_id,))
...
def parse_time_input(time_text: str) -> Dict[str, Any]:
    ...
    # 5. Проверяем "через X часов/минут"
    ...
    if future_match:
        ...
        logger.info(f"✅ Распознано будущее время: {time_str}")
        return {'time': time_str, 'type': 'future'}
...
def parse_reminder_text(text: str) -> Dict[str, Any]:
    ...
    # Извлекаем время - пробуем разные стратегии
    time_data = None
    
    # Стратегия 1: ищем время в начале текста (после ключевых слов)
    time_patterns = [
        r'(\d{1,2}:\d{2})',  # 14:30
        r'(\d{1,2})\s*(?:час\w*)?\s*(утра|вечера|ночи|дня)',  # 11 часов вечера
        r'(\d{1,2})\s+(утра|вечера|ночи)',  # 11 вечера
        r'(утром|днем|вечером|ночью)'  # утром
    ]
    ...
    for pattern in time_patterns:
        ...
        if match:
            ...
            time_data = parse_time_input(time_part)
            if time_data:
                # Удаляем распознанное время из текста
                clean_text = re.sub(re.escape(time_part), '', clean_text)
                break
    
    # Стратегия 2: если время не найдено, пробуем парсить весь чистый текст
    if not time_data:
        time_data = parse_time_input(clean_text)
        if time_data:
            # Если нашли время во всем тексте, удаляем его
            clean_text = re.sub(re.escape(clean_text), '', clean_text)
    ...
    if reminder_type == 'regular':
        ...
        if not days_of_week and reminder_type == 'regular':
    ...
    logger.info(f"✅ Результат парсинга: время={time_data['time']}, текст={reminder_text}, тип={reminder_type}, дни={days_of_week}")
...
async def use_template_start(update: Update, context: CallbackContext) -> int:
    ...
    for key, template in PLAN_TEMPLATES.items():
        keyboard.append([f"{template['name']} ({key})"])
...
async def select_template(update: Update, context: CallbackContext) -> int:
    ...
    if template_input == "❌ Отмена":
        await update.message.reply_text("❌ Отменено.", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    
    # Извлекаем ключ шаблона из текста
    template_key = None
    for key, template in PLAN_TEMPLATES.items():
        if template['name'] in template_input:
            template_key = key
            break
    
    if not template_key:
        for key in PLAN_TEMPLATES.keys():
            if key in template_input:
                template_key = key
                break
    
    if not template_key or template_key not in PLAN_TEMPLATES:
        await update.message.reply_text(
    ...
    context.user_data['template_name'] = PLAN_TEMPLATES[template_key]['name']
    
    await update.message.reply_text(
        f"✅ Выбран шаблон: {PLAN_TEMPLATES[template_key]['name']}\n\n"
...
async def select_user_for_template(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        return SELECT_DATE_FOR_TEMPLATE
...
async def select_date_for_template(update: Update, context: CallbackContext) -> int:
    ...
    success = generate_highly_personalized_plan(user_id, date_str, template_key)
    
    if success:
        await update.message.reply_text(
            f"✅ Шаблон успешно применен!\n\n"
            f"👤 Пользователь: {user_name}\n"
            f"📅 Дата: {date_str}\n"
            f"📋 Шаблон: {template_name}\n\n"
            f"План сохранен в Google Sheets и доступен пользователю."
        )
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text=f"🎉 У вас новый план на {date_str}!\n\n"
                     f"📋 {template_name}\n"
...
async def admin_templates(update: Update, context: CallbackContext):
    ...
    templates_text = "📋 ДОСТУПНЫЕ ШАБЛОНЫ ПЛАНОВ:\n\n"
    
    for key, template in PLAN_TEMPLATES.items():
        templates_text += f"🔸 {template['name']}\n"
        templates_text += f"   📝 {template['description']}\n"
        templates_text += f"   🎯 Задачи: {len(template.get('strategic_tasks', []))} стратегических, {len(template.get('critical_tasks', []))} критических\n"
        templates_text += f"   🔑 Ключ: {key}\n\n"
    
    templates_text += "💡 Команды:\n"
    templates_text += "/use_template - использовать шаблон\n"
    templates_text += "/add_plan - создать кастомный план"
...
async def analyze_user_profile_cmd(update: Update, context: CallbackContext):
    ...
    if not context.args:
        await update.message.reply_text("❌ Укажите ID пользователя: /analyze_profile <user_id>")
        return
    ...
    try:
        ...
        profile = analyze_user_profile(user_id_int)
        
        profile_text = f"🧠 ПРОФИЛЬ ПОЛЬЗОВАТЕЛЯ {user_id}:\n\n"
        
        profile_text += f"🎯 ТИП ЛИЧНОСТИ: {profile['personality_type']}\n"
        profile_text += f"💼 СТИЛЬ РАБОТЫ: {profile.get('work_style', {})}\n"
        profile_text += f"🎯 ЦЕЛЬ: {profile.get('main_goal', '')}\n\n"
        
        profile_text += f"⏰ ОПТИМАЛЬНОЕ ВРЕМЯ:\n"
        for activity, time in profile.get('optimal_times', {}).items():
            profile_text += f"  - {activity}: {time}\n"
        
        profile_text += f"\n🚧 ПРЕПЯТСТВИЯ: {', '.join(profile.get('obstacles', []))}\n"
        profile_text += f"🎯 МОТИВАЦИЯ: {', '.join(profile.get('motivation_triggers', []))}\n"
...
async def admin_users(update: Update, context: CallbackContext):
    ...
    for user in users:
        user_id, username, first_name, last_activity = user
...
# ========== ОБНОВЛЕННАЯ ФУНКЦИЯ PROGRESS_COMMAND ==========
